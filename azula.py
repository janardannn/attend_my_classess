...
from requirements.time_and_date import time_now # use time_now module to get current time as par IST time-zone
from requirements.time_and_date import today_date
...
class Firelord_Azula:
    ...
    def __init__(self):
        ...
        self.physics_attendance_today = 0
        
        self.chemistry_attendance_today = 0
        
        self.maths_attendance_today = 0
        
        self.english_attendance_today = 0
        
        ##self.notifier = ToastNotifier()

        self.log_file = open("/home/ironman/MySCripts/projekts/attend_my_classes/in_use/attend_my_classes/azula.log",'a+')
...
########################################################################################
    ...
    def attend_class(self,subject,class_url,class_timing,class_ending):
        ...
        # object's method to attend physics class

        class_name = subject

        self.driver.get(class_url) # opening physics classroom
        ...
        # finds join now button and clicks it
                time.sleep(10)
                self.driver.implicitly_wait(25)
                join_class = self.driver.find_element_by_xpath('//*[@id="yDmH0d"]/c-wiz/div/div/div[4]/div[3]/div/div/div[2]/div/div/div[2]/div/div[2]/div/div[1]/div[1]/span/span')
                join_class.click()

                print('Attending {} Class...      -- {} -- {}'.format(subject,time_now(),today_date()))
                self.log_file.write('{} class {} {} \n'.format(subject,time_now(),today_date()))

                ##self.notifier.show_toast('English Class','Attending English CLass',duration=15)
...
########################################################################################
    ...
    def attendance(self,subject):
        ...
        if subject == 'physics':
            ...
            class_link = class_info.physics_class_url
            teacher = class_info.physics_teacher
            attendance_line = 'Present Sir'

        elif subject == 'maths':
            check_1 = 'give your presence here'      
            check_2 = ' '

            class_link = class_info.maths_class_url
            teacher = class_info.maths_teacher
            attendance_line = 'Present Sir \nKV Jagiroad'
            ...
            class_link = class_info.chemistry_class_url
            teacher = class_info.chemistry_teacher
            attendance_line = 'Present Ma\'am \nKV Jagiroad'

        elif subject == 'english':
            check_1 = 'attendance'
            check_2 = ' '

            class_link = class_info.english_class_url
            teacher = class_info.english_teacher
            attendance_line = 'Present Ma\'am'
                        ...
                        try:
                            ...
                            if "AM" in time_of_post or "PM" in time_of_post:
                                ...
                                if check_1 in post_content and check_2 in post_content:
                                    ...
                                    if subject == 'physics':
                                        self.physics_attendance_today+=1       

                                        if self.physics_attendance_today == 1:
                                            ##self.notifier.show_toast("Physics Attendance","Azula has successfully commented on the latest attendance post in physics classroom",duration=15) 

                                            print('Physics Attendance marked present      -- {} -- {}'.format(time_now(),today_date()),end='\n')

                                            self.log_file.write('physics attendance {} {} \n'.format(time_now(),today_date()))

                                    elif subject == 'maths':
                                        self.maths_attendance_today+=1       

                                        if self.maths_attendance_today == 1:
                                            ##self.notifier.show_toast("Physics Attendance","Azula has successfully commented on the latest attendance post in physics classroom",duration=15) 

                                            print('Maths Attendance marked present      -- {} -- {}'.format(time_now(),today_date()),end='\n')

                                            self.log_file.write('maths attendance {} {} \n'.format(time_now(),today_date()))


                                    elif subject == 'chemistry':
                                        self.chemistry_attendance_today+=1       

                                        if self.chemistry_attendance_today == 1:
                                            ##self.notifier.show_toast("Physics Attendance","Azula has successfully commented on the latest attendance post in physics classroom",duration=15) 

                                            print('Chemistry Attendance marked present      -- {} -- {}'.format(time_now(),today_date()),end='\n')

                                            self.log_file.write('chemistry attendance {} {} \n'.format(time_now(),today_date()))

                                    elif subject == 'english':
                                        self.english_attendance_today+=1       

                                        if self.english_attendance_today == 1:
                                            ##self.notifier.show_toast("Physics Attendance","Azula has successfully commented on the latest attendance post in physics classroom",duration=15) 

                                            print('English Attendance marked present      -- {} -- {}'.format(time_now(),today_date()),end='\n')

                                            self.log_file.write('english attendance {} {} \n'.format(time_now(),today_date()))
...
def run():
    while True:
        ...
        # Physics attendance and class

        if time_now() > "08:05:00" and time_now() < class_info.physics_class_timing:
            if Azula.physics_attendance_today == 0:
                Azula.attendance('physics')

        elif time_now() > class_info.physics_class_timing and time_now() < class_info.physics_class_ending:
            Azula.attend_class("Physics",class_info.physics_class_url,class_info.physics_class_timing,class_info.physics_class_ending)

        # Chemistry attendance and class

        elif time_now() > "12:50:25" and time_now() < class_info.chemistry_class_timing:
            if Azula.chemistry_attendance_today == 0:
                Azula.attendance('chemistry')

        elif time_now() > class_info.chemistry_class_timing and time_now() < class_info.chemistry_class_ending:
            Azula.attend_class("Chemistry",class_info.chemistry_class_url,class_info.chemistry_class_timing,class_info.physics_class_ending)
        
        # Maths attendance and class

        elif time_now() > '10:05:25' and time_now() < class_info.maths_class_timing:
            if Azula.maths_attendance_today == 0:
                Azula.attendance('maths')

        elif time_now() > class_info.maths_class_timing and time_now() < class_info.maths_class_ending:
            Azula.attend_class("Maths",class_info.maths_class_url,class_info.maths_class_timing,class_info.maths_class_ending)

        # English attendance and class

        elif time_now() > '10:35:35' and time_now() < class_info.english_class_timing:
            if Azula.english_attendance_today == 0:
                Azula.attendance('english')

        elif time_now() > class_info.english_class_timing and time_now() < class_info.english_class_ending:
            Azula.attend_class("English",class_info.english_class_url,class_info.english_class_timing,class_info.english_class_ending)
        ...
        # time.sleep(25) to keep the cpu usage low if forever is running
        time.sleep(10)
